    ...
    async def list_paginated(self, page: int = 1, per_page: int = 50) -> Tuple[List[dict], int]:
        ...
        stmt = (
            select(
                ...
                func.count(IncomeTransaction.transaction_id).label("receive_count"),
                func.sum(IncomeTransaction.paid_total_credits).label("total_received"),
        ...
        count_stmt = select(func.count(func.distinct(IncomeTransaction.recipient_user_id)))
